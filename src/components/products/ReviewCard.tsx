...
export const ReviewCard: React.FC<ReviewCardProps> = ({
  ...
  return (
    <div className="bg-zinc-900 rounded-xl p-5 border border-zinc-800 hover:border-zinc-700 transition-colors">
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <h4 className="text-white font-semibold">{review.reviewerName}</h4>
            <StarRating rating={review.rating} size="sm" />
          </div>
          <p className="text-xs text-neutral-400">{formatDate(review.createdAt)}</p>
          {!review.isVisible && isAdmin && (
            <span className="inline-block mt-1 text-xs bg-red-950 text-red-400 px-2 py-1 rounded">
              Hidden
            </span>
          )}
        </div>
          ...
          <div className="flex items-center gap-2">
            {onToggleVisibility && (
              <button
                ...
                className="text-xs px-3 py-1 bg-zinc-800 hover:bg-zinc-700 text-neutral-300 rounded transition-colors"
              >
            ...
            {onDelete && (
              <button
                ...
                disabled={isDeleting}
                className="text-xs px-3 py-1 bg-red-950 hover:bg-red-900 text-red-400 rounded transition-colors disabled:opacity-50"
              >
      ...
      {/* Review Text */}
      <p className="text-neutral-300 text-sm leading-relaxed mb-3">{review.reviewText}</p>
      ...
      {review.images && review.images.length > 0 && (
        <div className="flex gap-2 mb-3 overflow-x-auto pb-2">
          {review.images.map((image, index) => (
            <div
              key={index}
              className="relative w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border border-zinc-800"
            >
              <Image
                src={image.src}
      ...
      {review.adminResponse && (
        <div className="mt-4 p-3 bg-zinc-950 border border-zinc-800 rounded-lg">
          ...
          <p className="text-sm text-neutral-300 leading-relaxed">{review.adminResponse.text}</p>
        </div>
      )}
        ...
        <div className="mt-4">
          {!showReplyForm ? (
            <button
              ...
              className="text-sm px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-neutral-300 rounded transition-colors"
            >
              Reply to Review
            </button>
          ) : (
