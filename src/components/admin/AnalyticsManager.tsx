      ...
      <div className="container mx-auto px-4 md:px-6 py-8">
        <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-6 text-center">
          ...
          <button
            ...
            className="mt-4 px-6 py-2 bg-gray-100 text-black rounded-lg hover:bg-gray-200 transition"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-8">
      ...
      <div className="flex flex-col gap-4 mb-6 md:mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-black mb-2">Analytics Dashboard</h1>
          <p className="text-sm md:text-base text-gray-600">Track website traffic and product performance</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base"
          >
            <Download size={18} />
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-6 mb-6 md:mb-8">
            {/* Total Product Views */}
            <div className="bg-white border border-gray-200 rounded-xl p-3 md:p-6">
              <div className="flex items-center justify-between mb-2 md:mb-4">
                <div className="p-1.5 md:p-3 bg-purple-500/10 rounded-lg">
                  <ShoppingBag className="text-purple-600" size={16} />
                </div>
              </div>
              <div className="text-xl md:text-3xl font-bold text-black mb-1">
                {formatNumber(analytics.totalProductViews)}
              </div>
              <div className="text-[10px] md:text-sm text-gray-600 leading-tight">Total Product Views</div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-3 md:p-6">
              <div className="flex items-center justify-between mb-2 md:mb-4">
                <div className="p-1.5 md:p-3 bg-green-500/10 rounded-lg">
                  <MousePointerClick className="text-green-600" size={16} />
                </div>
              </div>
              <div className="text-xl md:text-3xl font-bold text-black mb-1">
                {formatNumber(analytics.totalDMClicks)}
              </div>
              <div className="text-[10px] md:text-sm text-gray-600 leading-tight">Total DM Clicks</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 md:p-6">
              <h2 className="text-lg md:text-xl font-bold text-black mb-3 md:mb-4">Page Views</h2>
              {analytics.pageViewsByPage.length > 0 ? (
                  ...
                  {analytics.pageViewsByPage.map((pageView, index) => (
                    ...
                    >
                      <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                        <div className="flex items-center justify-center w-7 h-7 md:w-8 md:h-8 bg-gray-200 rounded-full text-xs font-semibold text-gray-700 flex-shrink-0">
                          {index + 1}
                        </div>
                ...
                <p className="text-gray-500 text-center py-8 text-sm">No page view data available</p>
              )}
            </div>

            {/* Top Products Table */}
            <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-4 md:p-6">
              <h2 className="text-lg md:text-xl font-bold text-black mb-3 md:mb-4">Top Products</h2>
              {analytics.topProducts.length > 0 ? (
                          ...
                          >
                            <td className="py-3 px-4">
                              <div className="flex items-center gap-3">
                                <div className="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full text-xs font-semibold text-gray-700">
                                  {index + 1}
                                </div>
                    ...
                    {analytics.topProducts.map((product, index) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-3">
                          <div className="flex items-center justify-center w-7 h-7 bg-gray-200 rounded-full text-xs font-semibold text-gray-700">
                            {index + 1}
                          </div>
                          <span className="text-black font-medium text-sm flex-1 truncate">
                            {product.productName}
                          </span>
                        </div>
                ...
                <p className="text-gray-500 text-center py-8 text-sm">No product data available</p>
              )}
            </div>
          </div>
        </>
